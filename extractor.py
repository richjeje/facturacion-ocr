...
def extract_text_from_image(file_path):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            results = easyocr_reader.readtext(img_array, detail=0)  # detail=0 para solo texto
            texto = " ".join(results)
            if texto.strip():
                logger.info(f"OCR EasyOCR exitoso en imagen {file_path} en intento {attempt + 1}")
                return texto, "EasyOCR"
            else:
                logger.warning(f"EasyOCR sin texto en imagen {file_path}, intento {attempt + 1}")
        except Exception as e:
            logger.warning(f"Error en EasyOCR de imagen {file_path}, intento {attempt + 1}: {e}")
            if attempt < max_retries:
                time.sleep(1 * (attempt + 1))  # Backoff
    logger.error(f"EasyOCR falló definitivamente en imagen {file_path}")
    raise ExtractionError(f"EasyOCR falló en imagen {file_path} tras {max_retries + 1} intentos")
