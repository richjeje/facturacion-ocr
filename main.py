...
from utils import ordenar_facturas_por_fecha, ExtractionError, ProcessingError, ValidationError
...
def process_single_file(archivo):
    ...
    try:
        ...
        if texto.strip():
            ...
            if resultado:
                logger.info(f"Factura procesada exitosamente: {resultado['Proveedor']} - {resultado['Folio']}")
                return resultado
        else:
...
max_files_input = input("¿Cuántos archivos deseas procesar? (presiona Enter para todos): ").strip()
...
archivos_filtrados = sorted([f for f in archivos if f.lower().endswith(('.pdf', '.jpg', '.jpeg', '.png'))])
if max_files is not None:
    archivos_filtrados = archivos_filtrados[:max_files]

# Procesamiento paralelo
with concurrent.futures.ThreadPoolExecutor(max_workers=min(4, len(archivos_filtrados))) as executor:
    futures = {executor.submit(process_single_file, archivo): archivo for archivo in archivos_filtrados}
...
if datos:
    ...
    orden = ["Fecha", "Proveedor", "Concepto", "Folio", "Subtotal", "IVA", "Total", "Metodo_Extraccion", "Confianza_Proveedor", "URL"]
