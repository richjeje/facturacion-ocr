...
def obtener_empresas_por_clasificacion():
    return {
        "ferreteria": ["ACEROS ALCALDE", "ACEROS Y MATERIALES SAN JORGE", "DELCASA FERRETERA", "DELCASA FERRETERIA",
                       "FIX FERRETERIAS", "HOME DEPOT MEXICO", "JULIETA CAMPOS GONZALEZ", "KALISCH FIERRO Y ACERO",
                       "LAURA CENTENO JIMENEZ", "LIDIA MONTSERRAT MENDOZA NIETO", "MASTER DEPOT",
                       "MULTIHERRAMIENTAS DEL BAJIO", "PLAFORAMA", "SOLUCIONES EN ALUMINIO", "TAISA ACABADOS",
                       "WIZARD ZONE MEXICO", "IMPERMEABILIZANTES Y MATERIALES DEL CENTRO"],
        "material electrico": ["COMERCIAL ELECTRICA", "DISTRIBUIDORA TAMEX", "DISTRIBUIDORA TAMEX SAPI DE CV",
                               "EUROELECTRICA", "EUGENIA MARIA ELENA VILLALPANDO"],
        "plomeria": ["LA INDUSTRIAL MEXICANA", "LIMSA SA DE CV", "PLOMERIA SELECTA", "PLASTICOS RUSSELL"],
...
meses_map = {"ene": "01","feb": "02","mar": "03","abr": "04","may": "05","jun": "06","jul": "07","ago": "08",
             "sep": "09","oct": "10","nov": "11","dic": "12"}

regex_fechas = [r"\d{1,2}/\d{1,2}/\d{4}", r"\d{1,2}-\d{1,2}-\d{4}", r"\d{4}-\d{1,2}-\d{1,2}"]
...
def procesar_texto(texto):
    if debe_omitir_factura(texto): return
    ...
    if match_texto:
        ...
        for patron in regex_fechas:
            ...
            if match: fecha = match.group(); break
    ...
    # --- Subtotal ---
    match_subtotal = re.search(r"sub\s*total[:\s]*([\d,]+\.\d{2})", texto, re.IGNORECASE)
    if match_subtotal:
    ...
    datos.append({"Fecha": fecha, "Proveedor": proveedor, "Concepto": tipo,
...
if not df.empty:
    ...
    try:
        if os.path.exists(excel_path):
            ...
            combined_df = ordenar_facturas_por_fecha(pd.concat([existing_df, df], ignore_index=True))
