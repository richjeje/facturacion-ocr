...
EMPRESAS_CONOCIDAS = _config.get('empresas_conocidas', [
...
CLASIFICACIONES_EMPRESAS = _config.get('clasificaciones_empresas', {
    "ferreteria": [
        "ACEROS ALCALDE", "ACEROS Y MATERIALES SAN JORGE", "DELCASA FERRETERA",
        "DELCASA FERRETERIA", "EDUARDO RODRIGUEZ LOPEZ", "FIX FERRETERIAS",
        "HOME DEPOT MEXICO", "JULIETA CAMPOS GONZALEZ", "KALISCH FIERRO Y ACERO",
        "LAURA CENTENO JIMENEZ", "LIDIA MONTSERRAT MENDOZA NIETO", "MASTER DEPOT",
        "MULTIHERRAMIENTAS DEL BAJIO", "PLAFORAMA", "SOLUCIONES EN ALUMINIO",
        "TAISA ACABADOS", "WIZARD ZONE MEXICO", "IMPERMEABILIZANTES Y MATERIALES DEL CENTRO","SERVICIO MAPRO SA DE CV",
        "MERCEDES ALEJANDRA LOPEZ MORALES", "OSCAR DELGADO CASAS",
        "ROBERTO BRAVO GUTIERREZ", "ISRAEL HERRERA SUBIAS", "LUIS FERNANDO MACIAS AVILA",
        "SURTIDORA DE ALAMBRES Y ACEROS DEL CENTRO", "AUTOZONE", "TIENDAS FIX",
        "FC FACIL DE CONTRUIR", "TAISA CONSTRUCCIONES", "MARTIN ORTEGA MATINEZ", "NATALIE MACIAS CAMARILLO",
        "GERSA"
    ],
    "material electrico": [
        "COMERCIAL ELECTRICA", "DISTRIBUIDORA TAMEX", "DISTRIBUIDORA TAMEX SAPI DE CV",
        "EUROELECTRICA", "EUGENIA MARIA ELENA VILLALPANDO", "INDUSTRIAL ELECTRICA"
    ],
    "plomeria": ["LA INDUSTRIAL MEXICANA", "LIMSA SA DE CV", "PLOMERIA SELECTA", "PLASTICOS RUSSELL", "ANA CECILIA ESQUIVEL COSSIO"],
    "maquinaria": ["RAUL SALAZAR MARTINEZ", "SERVICIO Y MAQUINARIA DE AGUASCALIENTES"],
    "refaccion": ["AUTO ZONE DE MEXICO"],
    "limpieza": ["DON PULCRO"],
    "voz y datos": ["STEREN", "TECNOTIENDAS AGUASCALIENTES", "CHELECTRONICA"],
    "papeleria": ["ANTONIO PADILLA RAMIREZ", "OFFICE DEPOT", "MARIA ISIDORA SOTO RAMIRES", "OPERADORA OMX"],
    "tornilleria": ["EDUARDO RODRIGUEZ LOPEZ DE LARA"],
    "pintura": ["COMEX", "PINTURAS MUÑOZ", "PINTURAS Y COMPLEMENTOS ORLOP SA DE CV"],
...
MESES_MAP = _config.get('meses_map', {"ene": "01","feb": "02","mar": "03","abr": "04","may": "05","jun": "06",
             "jul": "07","ago": "08","sep": "09","oct": "10","nov": "11","dic": "12",
             "nev": "11"})

REGEX_FECHAS = _config.get('regex_fechas', [
    r"\d{1,2}/\d{1,2}/\d{4}",
    r"\d{1,2}-\d{1,2}-\d{4}",
    r"\d{4}-\d{1,2}-\d{1,2}",
...
REGEX_FECHA_TEXTO = _config.get('regex_fecha_texto', r"(\d{1,2})/([a-z0-9]{3})\.?/(\d{4})(?:\s+\d{1,2}:\d{2}:\d{2})?")

# --- Configuración de folios ---
FOLIO_PATTERNS = _config.get('folio_patterns', {
...
FOLIO_PATTERNS_GENERAL = _config.get('folio_patterns_general', [
    r"Folio:\s*([0-9]+)",
    r"Folio[^f]*([0-9]+)",
    r"No\.?\s*Folio[^f]*([0-9]+)",
...
FIX_ADDRESSES = _config.get('fix_addresses', [
...
# --- Regex adicionales ---
REGEX_SUBTOTALS = _config.get('regex_subtotals', [
    r"sub\s*total[^\d]*\$?(\d+(?:[,.]\d+)*)",
    r"subtotal\s*:\s*\$?(\d+(?:[,.]\d+)*)",
    r"sub\s*total\s*\$?(\d+(?:[,.]\d+)*)",
    r"sub-?tot[^\d]*\$?(\d+(?:[,.]\d+)*)",
...
REGEX_TOTALS = _config.get('regex_totals', [
    r"total[^\d]*\$?\s*(\d+(?:[,.]\d+)*)",
    r"total\s*:\s*\$?\s*(\d+(?:[,.]\d+)*)",
...
def setup_logging():
    ...
    file_handler = RotatingFileHandler(os.path.join(log_dir, "processing.log"), maxBytes=5*1024*1024, backupCount=5)
